...
export function VideoPreview() {
  ...
  useEffect(() => {
    const generateVideo = async () => {
      try {
        await generateVideoPreview("current-content");
  ...
  const handleRegenerateVideo = async () => {
    ...
    try {
      await regenerateVideoPreview("current-content");
      setIsGenerating(false);
    } catch (error) {
  ...
  const handleDownload = async () => {
    try {
      await downloadVideo("current-content");
    } catch (error) {
  ...
  const videoReady = !isGenerating && videoPreview?.status === "ready";
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="xl:col-span-3">
            <Card className="overflow-hidden shadow-2xl border-0">
              <CardContent className="p-0">
                <div className="aspect-[9/16] max-w-md mx-auto bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 relative overflow-hidden">
                  {!videoReady ? (
            ...
            {videoReady && (
              <div className="flex justify-center gap-4 mt-6">
                ...
                >
                  ...
                  {loadingVideoPreview ? "Пересоздание..." : "Пересоздать"}
                </Button>
              </div>
            )}
          </div>
            ...
            <Card className="border-0 shadow-lg">
              <CardContent className="p-6">
                ...
                <div className="text-sm text-gray-600 leading-relaxed">
                  {videoPreview?.script ? (
                    <p>{videoPreview.script.substring(0, 200)}...</p>
                  ) : (
        ...
        <div className="flex justify-between items-center mt-12 pt-8 border-t border-gray-200">
          <Button
            variant="outline"
            size="lg"
            onClick={() => navigate(CONTENT_ROUTE_SCRIPT)}
            className="px-8"
          >
